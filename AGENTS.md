...
- **Never put `DEVELOPER_KEYS_ADMIN_SECRET` in client code or `VITE_*` env.**
- **Never trust client-supplied `subjectId` for mint/list/revoke** — only the signed session cookie.
- Do not log API key tokens or admin secrets.
...
| `src/server/` | Hono BFF (session + keys proxy) |
| `test/` | Vitest |
...
Portal exposes only `/api/session`, `/api/keys`, `/api/keys/:id` to the browser.
